"""Tape substrate — taker-tape aggregation and cross-venue flow math.

Owns the canonical ``summarize`` flow summary (the trade-tape half plus the
top-of-book metrics it reports), bucketed CVD, bucket correlation, turnover
share, and price-bucketed flow. ``microprice_skew_bps`` lives here because
``summarize`` computes it — substrates never import each other, so the helper
co-locates with its consumer.

Pure math over normalized trades (``ts``, ``price``, ``qty``,
``is_buyer_maker``) and books (``bids``/``asks`` as ``(price, qty)``).
"""
