"""DELTA variable: signed -2..+2 combo of wall imbalance + flow alignment.

Lifted from legacy ``consolidated.py:107`` and ``delta_calc.py:96``. Splits the
orderbook in ``band_step`` increments across ``±half_range`` of price, computes
per-band wall imbalance, sums it into a single range imbalance, and combines it
with the latest taker buy/sell flow alignment to produce the signed DELTA
verdict variable.

All functions are pure over normalized inputs (book dict + taker_buy_sell list);
