"""Volume profile: price-bucketed volume distribution with POC / value area.

Lifted from legacy ``long_term_flow.py`` so the volume-profile read (POC,
VAH/VAL value area, HVN/LVN) is a reusable canonical primitive.

Pure math: takes normalized trades (``ts``, ``price``, ``qty``,
``is_buyer_maker``) and returns deterministic structures.
"""
