...
PHASE_GUIDANCE: dict[str, str] = {
    ...
    "P5": ("PHASE P5 — DERIVE: call memory.recall_paper FIRST (ground H0/H1 in Cont 1011.6402 facts), "
           ...
           "ForecastResult (schema-checked features, forward target, train-only OOS, calibration gate, "
           "native/long-horizon classification, and preserved Route A/B disagreement). The lower-level "
           "calc.feature.build → calc.forward.join → calc.forward.fit → calc.forward.distribution tools "
           "are diagnostics only. Then use calc.forward.scenario for P(T)/P(S) when calibrated; legacy "
           "calc.scenario.evaluate remains a linear flow-requirement scenario. → "
