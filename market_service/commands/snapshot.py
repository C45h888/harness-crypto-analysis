...
async def run(symbol: str, trades: int, depth: int, window: int) -> dict:
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.trades <= 0 or args.depth <= 0 or args.window <= 0:
        parser.error("--trades, --depth, and --window must be positive")
    try:
