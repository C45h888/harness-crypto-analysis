...
time. One mount point covers host shell, Docker, and the harness command:

```bash
# VSCode shell (uses .venv; repo .env is loaded non-clobbering)
./nooa market envelope SOLUSDT --latest            # read collated envelope
...
./nooa market refresh SOLUSDT --all                # bounded canonical refresh
...
# Mounted through harness.py itself
...
The `market` group speaks to the canonical stores (Redis primary read,
Postgres durable ledger) through the python objects in
`market_service/nooa_harness/` — `agents.py` defines the ControllerAgent +
specialist nodes, `suite.py`/`runner.py` compose them, `memory.py` persists
MemoryNode. The CLI stays read-only against canonical state; only the
